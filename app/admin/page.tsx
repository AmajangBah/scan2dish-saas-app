...
export default async function AdminDashboard() {
  ...
  const { data: overdueRestaurants } = await supabase
    .from("restaurants")
    .select("id, name, total_commission_owed, total_commission_paid, last_payment_date, menu_enabled")
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <MetricCard
          ...
          icon={<DollarSign />}
          color="orange"
          subtitle={`$${Number(m.commission_last_30d || 0).toFixed(2)} generated (30d)`}
        />
      </div>
...
function ActivityIcon({ action }: { action: string }) {
  ...
  if (action.includes("payment")) {
    return <DollarSign className="h-5 w-5 text-green-500 flex-shrink-0" />;
  }
  return <AlertTriangle className="h-5 w-5 text-gray-400 flex-shrink-0" />;
}
